...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen flex flex-col w-full bg-white dark:bg-[#090E17] transition-colors duration-300 pb-safe overflow-x-hidden text-gray-800 dark:text-gray-200">
      ...
      {/* ✅ BLOG SECTION */}
      <section className="w-full bg-white dark:bg-[#090E17] py-20 border-t border-gray-100 dark:border-gray-800/60">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight mb-4">
              Learn Before You <span className="text-blue-600 dark:text-blue-400">Trade</span>
            </h2>
            <p className="text-gray-600 dark:text-gray-400 text-base sm:text-lg max-w-2xl mx-auto">
              Practical guides for Bangladeshi investors — from opening your first BO account to understanding the DSE.
            </p>
          </div>
            ...
            {[
              {
                ...
                description: 'Learn the exact steps, required documents, and fees to open a BO account and start trading on the DSE.',
              },
              {
                href: '/blog/top-dse-stock-brokers-2026',
                tag: 'Broker Comparison',
                title: 'Compare Top DSE Stock Brokers in Bangladesh',
                description: 'Side-by-side comparison of BO account fees, commission rates, and platforms from leading DSE brokers.',
              },
              {
                href: '/blog',
                tag: 'All Articles',
                title: 'More Guides on the Blog',
                description: 'Browse all our articles on DSE investing, trading strategies, and the Bangladeshi stock market.',
                isMore: true,
              },
            ].map((post) => (
              <Link
                key={post.href}
                href={post.href}
                className={`group flex flex-col bg-gray-50 dark:bg-[#1A1F26] border rounded-3xl p-6 sm:p-8 hover:-translate-y-1 hover:shadow-md transition-all duration-300 ${
                  post.isMore
                    ? 'border-dashed border-gray-300 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 justify-center items-center text-center'
                    : 'border-gray-100 dark:border-gray-800 hover:border-blue-400 dark:hover:border-blue-600'
                }`}
              >
                <span className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-3">
                  {post.tag}
                </span>
                <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug">
                  {post.title}
                </h3>
                ...
                {!post.isMore && (
                  <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 dark:text-blue-400 group-hover:gap-2.5 transition-all">
          ...
          <div className="text-center">
            <Link
              href="/blog"
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl border-2 border-blue-600 dark:border-blue-500 text-blue-600 dark:text-blue-400 font-bold text-base hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 dark:hover:text-white transition-all duration-200"
            >
